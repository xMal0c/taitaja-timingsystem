...
export default function MainWindow() {
  ...
  //Timer function to update and start the time
  const [start, setStart] = useState(false);
  const [timer, setTimer] = useState(Array(6.0).fill(0));
  ...
  return (
    <div className="flex py-1 px-1 text-white w-9/12 items-strech relative">
      ...
      <div className="container bg-[#292c34] w-4/12 rounded-tl-xl rounded-bl-sm text-center mx-auto py-2 px-2">
        ...
        <div className="bg-[#4f515a] m-2 rounded-xl py-2 my-10 flex justify-center items-center">
          <div className="flex-1 flex justify-center">
            <p className="subpixel-antialiased">Pallo</p>
          </div>
          <p className="px-2 subpixel-antialiased text-sm text-stone-400">|</p>
          <div className="flex-1 flex justify-center">
            <p className="subpixel-antialiased">Ei</p>
          </div>
        </div>
      ...
      <div className="container bg-[#2b2f39] flex-auto rounded-tr-xl rounded-br-sm mx-auto text-center py-2 px-2 relative before:absolute before:left-0 before:top-8 before:bottom-8 before:w-px before:bg-green-500 before:content-['']">
        ...
        <div className="px-2 py-2 flex justify-around items-center">
          {Array.from({ length: 6 }).map((_, index) => (
            <div
              key={index}
              className="bg-[#4f515a] hover:bg-[#414552] rounded-t-2xl rounded-b-md py-2 px-9"
            >
              <p className="subpixel-antialiased cursor-default font-medium text-[#e1e1e2]">
                Joukkue{index + 1}
              </p>
            </div>
          ))}
        ...
        {/* Timer divs */}
        <div className="bg-[#4f515a] m-2 rounded-xl my-8 flex justify-around items-center">
          {timer.map((time, index) => (
            <div key={index} className="min-w-0 py-2 px-1 flex-1">
              <p className="antialiased">{formatTime(time)}</p>
            </div>
          ))}
        </div>

        <div className="bg-[#4f515a] m-2 rounded-xl my-10 flex justify-around items-center">
          {timer.map((time, index) => (
            <div key={index} className="min-w-0 py-2 px-1 flex-1">
              <p className="antialiased">{formatTime(time)}</p>
            </div>
        ...
        <div className="bg-[#4f515a] m-2 rounded-xl my-9 flex justify-around items-center">
          {timer.map((time, index) => (
            <div key={index} className="min-w-0 py-2 px-1 flex-1">
              <p className="antialiased">{formatTime(time)}</p>
            </div>
        ...
        <div className="text-center ">
          <div className="flex justify-center gap-2 mt-20 mb-4 w-full">
            ...
            <button className="bg-sky-400 hover:bg-sky-600 py-2 rounded-xl w-2/12">
              TUO
            </button>
          </div>
        </div>
